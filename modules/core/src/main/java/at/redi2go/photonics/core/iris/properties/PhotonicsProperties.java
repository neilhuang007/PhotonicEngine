...
public interface PhotonicsProperties extends PropertyDefines {
    @DefaultValue("false")
    boolean isEnabled();
    ...
    default void defineProperties(PhotonicsProperties properties) {
        // The CPU voxel flags and GPU traversal must use the same mode.
        // An enum define alone cannot express VOXEL's two enabled switches.
        switch (getTransparencyMode()) {
            case BLOCK -> stringDefine("PH_USE_TRANSPARENCY", "");
            case VOXEL -> {
                stringDefine("PH_USE_TRANSPARENCY", "");
                stringDefine("PH_FULL_TRANSPARENCY", "");
            }
        }
    }
}
