...
public class RestirPipeline extends PhotonicsPipeline {
    ...
    private ReservoirSplattingRendering reservoirSplatting;
    ...
    public RestirPipeline(
        ...
        this.denoiserPasses = restirProperties.getDenoiserPasses();
        ...
        if (Boolean.getBoolean("photonics.traceLighting")) lightingProbePipeline(irisPipeline);
    ...
    private void restirDiPipeline(IrisPipeline irisPipeline) {
        ...
        var framebuffer = irisPipeline.newFramebuffer(properties.getRenderScale())
                .addAttachment("restir_direct_reservoirs0", ITextureFormat.rgb32ui(), CREATE_SAMPLER | FLIP)
                .addAttachment("restir_direct_reservoirs1", ITextureFormat.rgb32f(), CREATE_SAMPLER | FLIP)
                .addAttachment("restir_direct_candidates", ITextureFormat.rgba32ui(), CREATE_SAMPLER)
                .addAttachment("di_output", ITextureFormat.rgba32f(), CREATE_SAMPLER)
                .build(this::registerComponent);
        reservoirSplatting = registerComponent(new ReservoirSplattingRendering(
                framebuffer, lightList::contentGeneration, worldCompiler::contentGeneration));

        var reGIR = registerComponent(new ReGIRRendering(restirProperties.getReGIRProperties()));
        var powerSampler = registerComponent(new LightPowerSampler(properties.getLightListProperties().getSize()));
        var builder = irisPipeline.newRenderer()
                .debugGroup("restir di")
                .withFragmentPrefix("/photonics/rendering/restir/direct/passes/")
                .withFramebuffer(framebuffer)
                .thenFlip(framebuffer);
        builder = powerSampler.addPreparationPasses(builder, this::isPowerRISPreparationEnabled)
                .computePass("build ReGIR", ReGIRRendering.BUILD_SHADER,
                        reGIR.buildWorkGroupsX(), reGIR.buildWorkGroupsY(), 1, this::isReGIREnabled)
                .thenShaderStorageBarrier(this::isReGIREnabled)
                .deferredPass("initial direct", "di0_initial_direct.fsh", null);
        reservoirSplatting.addPasses(builder, this::isBlockLightEnabled, properties.getRenderScale())
                .deferredPass("temporal splatting", "di1_temporal_reuse.fsh", null)
                .thenShaderStorageBarrier()
                .thenFlip(framebuffer)
                .when(this::isDiSpatialReuseEnabled, b0 -> {
                    b0.deferredPass("spatial reuse", "di2_spatial_reuse.fsh", null);
                    b0.thenShaderStorageBarrier();
                    b0.thenRun(reservoirSplatting::promoteSpatialOutput);
                    b0.thenFlip(framebuffer);
                })
                .deferredPass("resolve direct", "di3_validate_visibility.fsh", null)
                .build(this::registerRenderer);
    }

    private void lightingProbePipeline(IrisPipeline irisPipeline) {
        var framebuffer = irisPipeline.newFramebuffer(9, 256)
                .addAttachment("probe_unoccluded", ITextureFormat.rgba32f(), CREATE_SAMPLER)
                .addAttachment("probe_visible", ITextureFormat.rgba32f(), CREATE_SAMPLER)
                .addAttachment("probe_first_hit", ITextureFormat.rgba32f(), CREATE_SAMPLER)
                .addAttachment("probe_sky_visibility", ITextureFormat.rgba32f(), CREATE_SAMPLER)
                .addAttachment("probe_roof_hit", ITextureFormat.rgba32f(), CREATE_SAMPLER)
                .addAttachment("probe_sun_hit", ITextureFormat.rgba32f(), CREATE_SAMPLER)
                .addAttachment("probe_sun_direction", ITextureFormat.rgba32f(), CREATE_SAMPLER)
                .addAttachment("probe_primary_hit", ITextureFormat.rgba32f(), CREATE_SAMPLER)
                .build(this::registerComponent);
        irisPipeline.newRenderer().debugGroup("diagnostic light probes")
                .withFramebuffer(framebuffer)
                .deferredPass("diagnostic light probes", "/photonics/rendering/diagnostics/light_probes.fsh", null)
                .build(this::registerRenderer);
    }
    ...
    private void restirGiPipeline(IrisPipeline irisPipeline) {
        ...
        var framebuffer = irisPipeline.newFramebuffer(properties.getRenderScale())
                ...
                .addAttachment("gi_candidates0", ITextureFormat.rgba32f(), CREATE_SAMPLER)
                .addAttachment("gi_candidates1", ITextureFormat.rgb32ui(), CREATE_SAMPLER)
                .build(this::registerComponent);
    ...
    private void svgfPipeline(IrisPipeline irisPipeline) {
        ...
        irisPipeline.newRenderer()
                ...
                .repeat(denoiserPasses, (i, b0) -> {
                    // SVGF filters fine-to-coarse. Do not add hidden wide-radius
                    // passes for the hand, or skip the one-pixel first step.
                    int index = i;
    ...
    public ReservoirSplattingRendering.HistorySnapshot reservoirSplattingHistorySnapshot() {
        return reservoirSplatting == null ? null : reservoirSplatting.historySnapshot();
    }

    public boolean isReGIREnabled() {
        return isBlockLightEnabled() && restirProperties.getReGIRProperties().getReGIRMode() != ReGIRMode.DISABLED;
    }

    public boolean isPowerRISPreparationEnabled() {
        var regir = restirProperties.getReGIRProperties();
        return isReGIREnabled() && (
                regir.getReGIRLocalLightPresamplingMode() == ReGIRLocalLightPresamplingMode.POWER_RIS ||
                regir.getReGIRLocalLightFallbackMode() == ReGIRLocalLightFallbackMode.POWER_RIS);
    }
