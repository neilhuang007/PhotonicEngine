...
public class Pipelines {
    ...
    public static void fragData(PhotonicsPipeline ext, PhotonicsProperties properties, IrisPipeline irisPipeline) {
        var jitter = irisPipeline.newFramebuffer(1, 1)
                .addAttachment("ph_frame_jitter", ITextureFormat.rg32f(), CREATE_SAMPLER | FLIP)
                .build(ext::registerComponent);
        irisPipeline.newRenderer()
                .debugGroup("frame jitter")
                .withFramebuffer(jitter)
                .thenFlip(jitter)
                .deferredPass("record frame jitter", "/photonics/rendering/frag/passes/j0_record_jitter.fsh", null)
