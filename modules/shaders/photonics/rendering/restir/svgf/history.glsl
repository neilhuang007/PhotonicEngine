...
bool sample_history_is_valid(SampleHistory history) {
    return !any(isnan(history.lighting)) && !any(isinf(history.lighting)) &&
            !any(isnan(history.variance)) && !any(isinf(history.variance)) &&
            !isnan(history.visibility) && !isinf(history.visibility);
}
...
void sample_history_reproject(out SampleHistory temporal_history, out vec4 fast_history) {
    ...
    for (int i = 0; i < weights.length(); i++) {
        ...
        if (any(lessThan(p, ivec2(0))) ||
                any(greaterThanEqual(p, textureSize(prev_diffuse_history, 0)))) continue;

        FragData prev_frag;
        frag_data_load_previous(prev_frag, p);
        if (!frag_data_is_in_world(prev_frag) ||
                frag_data_is_hand(prev_frag) != frag_is_hand ||
                frag_data_is_light_transmissive(prev_frag) != frag_is_light_transmissive) continue;
        ...
        if (!sample_history_is_valid(result) ||
                any(isnan(fast_sample)) || any(isinf(fast_sample))) continue;
...
void sample_history_add_sample(inout SampleHistory history, inout vec4 fast_history, vec4 smple) {
    ...
    history.visibility = smple.a;
    history.variance = vec4(0.0f);
    fast_history = history.lighting;
    return;
#endif
