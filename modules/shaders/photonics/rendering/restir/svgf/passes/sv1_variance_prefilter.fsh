    ...
    if (frag_is_in_world) {
        ...
        for (int i = 0; i < 9; i++) {
            ...
            if (any(lessThan(p, ivec2(0))) ||
                    any(greaterThanEqual(p, textureSize(diffuse_history, 0)))) continue;
        ...
        // A bright emitter/edge is not a firefly simply because it is brighter
        // than its neighbors. Preserve the center estimate.
        smple.color = clamp(center.rgb, -65504.0f, 65504.0f);
