...
// The legacy Shrimple traversal helper remains available because old native
// packs can still provide internal/tracing/simple.glsl.
void ray_iter_apply_transparency(inout vec4 accumulator, vec4 albedo) {
    if (accumulator.a != 0.0f) {
        float mix_factor = (1.0f - accumulator.a) * albedo.a;
        accumulator.rgb = mix(accumulator.rgb, albedo.rgb, mix_factor);
        accumulator.a += mix_factor;
    } else {
        accumulator = albedo;
