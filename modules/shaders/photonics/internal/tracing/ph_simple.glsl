bool trace_light_vis(
    ...
    while (true) {
        ...
        if (ray_result_is_transparent(result)) {
            ...
            light_transmittance *= voxel_data_visibility_transmittance(
                voxel_data,
                albedo
            );
    ...
    tint_color = running_tint_color.a == 0.0f
            ? vec3(1.0f)
...
// These primary paths represent the opaque G-buffer. Minecraft composites its
// TRANSLUCENT block layer later, so glass must not replace/occlude the receiver
// here. Light and GI visibility retain their separate attenuation traversal.
RayResult trace_opaque_camera_hit(inout RayIterator ray) {
    while (true) {
        RayResult hit = ray_iter_next(ray);
        if (!ray_result_is_hit(hit) ||
                !voxel_data_is_light_transmissive(ray_result_voxel_data(hit))) {
            return hit;
        }
...
bool trace_segment_visibility(
    ...
    while (true) {
        RayResult result = trace_opaque_camera_hit(ray);
